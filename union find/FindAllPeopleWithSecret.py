...
Sort meetings by time and group meetings happening at the same time.

For each group of meetings happening at the same time, union each pair of people in a meeting, and then
check if eather p1 or p2 is connected to person 0, to check if they know the sected, if they don't know
the sectet, reset their parent to themselves.

Use rank to optimize union operations and path compression to optimize find operations to nearly constant time.

Time complexity: O(MlogM + N) where M is the number of meetings and N is the number of people.
...
class DSU:
        ...
        def get_members(self, parent: int) -> List[int]:
            return [i for i in range(len(self.parents)) if self.is_connected(i, parent)]
...
class Solution:
    def findAllPeople(self, n: int, meetings: List[List[int]], firstPerson: int) -> List[int]:
        ...
        # Group meetings happening at the same time
